...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const addCandidate = useCallback(async (candidate: Candidate) => { if (!user) throw new Error("Usuário não autenticado."); const { data, error } = await supabase.from('candidates').insert({ user_id: user.id, data: candidate }).select('id').single(); if (error) throw error; if (data) { setCandidates(prev => [{ ...candidate, db_id: data.id }, ...prev]); } }, [user, session]);
  const updateCandidate = useCallback(async (id: string, updates: Partial<Candidate>) => { if (!user) throw new Error("Usuário não autenticado."); const c = candidates.find(c => c.id === id); if (!c || !c.db_id) throw new Error("Candidato não encontrado"); const updated = { ...c, ...updates }; const { db_id, ...dataToUpdate } = updated; const { error } = await supabase.from('candidates').update({ data: dataToUpdate }).match({ id: c.db_id, user_id: user.id }); if (error) throw error; setCandidates(prev => prev.map(p => p.id === id ? updated : p)); }, [user, session, candidates]);
  const deleteCandidate = useCallback(async (id: string) => { if (!user) throw new Error("Usuário não autenticado."); const c = candidates.find(c => c.id === id); if (!c || !c.db_id) throw new Error("Candidato não encontrado"); const { error } = await supabase.from('candidates').delete().match({ id: c.db_id, user_id: user.id }); if (error) throw error; setCandidates(prev => prev.filter(p => p.id !== id)); }, [user, session, candidates]);
  
  const addTeamMember = useCallback(async (member: TeamMember) => { if (!user) throw new Error("Usuário não autenticado."); const { data, error } = await supabase.from('team_members').insert({ user_id: user.id, data: member }).select('id').single(); if (error) throw error; if (data) { setTeamMembers(prev => [...prev, { ...member, db_id: data.id }]); } }, [user, session]);
  const updateTeamMember = useCallback(async (id: string, updates: Partial<TeamMember>) => { if (!user) throw new Error("Usuário não autenticado."); const m = teamMembers.find(m => m.id === id); if (!m || !m.db_id) throw new Error("Membro não encontrado"); const updated = { ...m, ...updates }; const { db_id, ...dataToUpdate } = updated; const { error } = await supabase.from('team_members').update({ data: dataToUpdate }).match({ id: m.db_id, user_id: user.id }); if (error) throw error; setTeamMembers(prev => prev.map(p => p.id === id ? updated : p)); }, [user, session, teamMembers]);
  const deleteTeamMember = useCallback(async (id: string) => { if (!user) throw new Error("Usuário não autenticado."); const m = teamMembers.find(m => m.id === id); if (!m || !m.db_id) throw new Error("Membro não encontrado"); const { error } = await supabase.from('team_members').delete().match({ id: m.db_id, user_id: user.id }); if (error) throw error; setTeamMembers(prev => prev.filter(p => p.id !== id)); }, [user, session, teamMembers]);

  const addCommission = useCallback(async (commission: Commission) => { if (!user) throw new Error("Usuário não autenticado."); const { data, error } = await supabase.from('commissions').insert({ user_id: user.id, data: commission }).select('id').single(); if (error) throw error; if (data) { setCommissions(prev => [{ ...commission, db_id: data.id }, ...prev].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())); } }, [user, session]);
  const updateCommission = useCallback(async (id: string, updates: Partial<Commission>) => { if (!user) throw new Error("Usuário não autenticado."); const c = commissions.find(c => c.id === id); if (!c || !c.db_id) throw new Error("Comissão não encontrada"); const updated = { ...c, ...updates }; const { db_id, ...dataToUpdate } = updated; const { error } = await supabase.from('commissions').update({ data: dataToUpdate }).match({ id: c.db_id, user_id: user.id }); if (error) throw error; setCommissions(prev => prev.map(p => p.id === id ? updated : p).sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())); }, [user, session, commissions]);
  const deleteCommission = useCallback(async (id: string) => { if (!user) throw new Error("Usuário não autenticado."); const c = commissions.find(c => c.id === id); if (!c || !c.db_id) throw new Error("Comissão não encontrada"); const { error } = await supabase.from('commissions').delete().match({ id: c.db_id, user_id: user.id }); if (error) throw error; setCommissions(prev => prev.filter(p => p.id !== id)); }, [user, session, commissions]);
  
  const addSupportMaterial = useCallback(async (material: SupportMaterial) => { if (!user) throw new Error("Usuário não autenticado."); const { data, error } = await supabase.from('support_materials').insert({ user_id: user.id, data: material }).select('id').single(); if (error) throw error; if (data) { setSupportMaterials(prev => [{ ...material, db_id: data.id }, ...prev]); } }, [user, session]);
  const deleteSupportMaterial = useCallback(async (id: string) => { if (!user) throw new Error("Usuário não autenticado."); const m = supportMaterials.find(m => m.id === id); if (!m || !m.db_id) throw new Error("Material não encontrado"); const { error } = await supabase.from('support_materials').delete().match({ id: m.db_id, user_id: user.id }); if (error) throw error; setSupportMaterials(prev => prev.filter(p => p.id !== id)); }, [user, session, supportMaterials]);
