...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const addCandidate = async (candidate: Candidate) => { if (!user) return; try { const { data, error } = await supabase.from('candidates').insert({ user_id: user.id, data: candidate }).select('data'); if (error) throw error; if (data) setCandidates(prev => [data[0].data, ...prev]); } catch (error) { handleApiError(error, 'candidato'); } };
  const updateCandidate = async (id: string, updates: Partial<Candidate>) => { if (!user) return; const c = candidates.find(c => c.id === id); if (!c) return; const updated = { ...c, ...updates }; try { const { data, error } = await supabase.from('candidates').update({ data: updated }).match({ 'data->>id': id, user_id: user.id }).select('data'); if (error) throw error; if (data) setCandidates(prev => prev.map(p => p.id === id ? data[0].data : p)); } catch (error) { handleApiError(error, 'candidato'); } };
  ...
  const addTeamMember = async (member: TeamMember) => { if (!user) return; try { const { data, error } = await supabase.from('team_members').insert({ user_id: user.id, data: member }).select('data'); if (error) throw error; if (data) setTeamMembers(prev => [...prev, data[0].data]); } catch (error) { handleApiError(error, 'membro da equipe'); } };
  const updateTeamMember = async (id: string, updates: Partial<TeamMember>) => { if (!user) return; const m = teamMembers.find(m => m.id === id); if (!m) return; const updated = { ...m, ...updates }; try { const { data, error } = await supabase.from('team_members').update({ data: updated }).match({ 'data->>id': id, user_id: user.id }).select('data'); if (error) throw error; if (data) setTeamMembers(prev => prev.map(p => p.id === id ? data[0].data : p)); } catch (error) { handleApiError(error, 'membro da equipe'); } };
  ...
  const addCommission = async (commission: Commission) => { if (!user) return; try { const { data, error } = await supabase.from('commissions').insert({ user_id: user.id, data: commission }).select('data'); if (error) throw error; if (data) setCommissions(prev => [data[0].data, ...prev].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())); } catch (error) { handleApiError(error, 'comissão'); } };
  const updateCommission = async (id: string, updates: Partial<Commission>) => { if (!user) return; const c = commissions.find(c => c.id === id); if (!c) return; const updated = { ...c, ...updates }; try { const { data, error } = await supabase.from('commissions').update({ data: updated }).match({ 'data->>id': id, user_id: user.id }).select('data'); if (error) throw error; if (data) setCommissions(prev => prev.map(p => p.id === id ? data[0].data : p).sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())); } catch (error) { handleApiError(error, 'comissão'); } };
  ...
  const addSupportMaterial = async (material: SupportMaterial) => { if (!user) return; try { const { data, error } = await supabase.from('support_materials').insert({ user_id: user.id, data: material }).select('data'); if (error) throw error; if (data) setSupportMaterials(prev => [data[0].data, ...prev]); } catch (error) { handleApiError(error, 'material'); } };
