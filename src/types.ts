...
export interface LeadTask {
  ...
  completed_at?: string;
  created_at: string;
  type: 'task' | 'meeting'; // RE-ADDED: 'meeting' type
  meeting_start_time?: string; // RE-ADDED: Meeting fields
  meeting_end_time?: string;   // RE-ADDED: Meeting fields
  manager_id?: string; // RE-ADDED: Meeting fields
  manager_invitation_status?: 'pending' | 'accepted' | 'declined'; // RE-ADDED: Meeting fields
  updated_at?: string; // RE-ADDED: updated_at
}
...
// RE-ADDED: ConsultantEvent interface
export interface ConsultantEvent {
  ...
  start_time: string;
  end_time: string;
  event_type: 'personal_task' | 'training' | 'other'; // RE-ADDED: event_type
  created_at: string;
}
...
export interface AppContextType {
  ...
  formFiles: FormFile[];
  consultantEvents: ConsultantEvent[]; // RE-ADDED: ConsultantEvent
  ...
  addConsultantEvent: (event: Omit<ConsultantEvent, 'id' | 'user_id' | 'created_at'>) => Promise<ConsultantEvent>; // RE-ADDED: Function to add consultant event
  updateConsultantEvent: (id: string, updates: Partial<ConsultantEvent>) => Promise<ConsultantEvent>; // RE-ADDED: Function to update consultant event
  deleteConsultantEvent: (id: string) => Promise<void>; // RE-ADDED: Function to delete consultant event
}
...
const DEFAULT_APP_CONFIG_DATA = {
  ...
  salesOrigins: ['WhatsApp', 'Frio', 'Instagram', 'Networking', 'Tráfego Pago', 'Indicação'],
...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const resetLocalState = () => {
    ...
    setFormCadastros([]);
    setFormFiles([]);
    setConsultantEvents([]); // RE-ADDED: Reset consultant events
    setNotifications([]);
    ...
    const fetchData = async (userId: string) => {
      ...
      try {
        ...
        const [
          ...
          formCadastrosData,
          formFilesData,
          consultantEventsData, // RE-ADDED: Busca de eventos do consultor
        ] = await Promise.all([
          ...
          (async () => { try { return await supabase.from('consultant_events').select('*').eq('user_id', userId); } catch (e) { console.error("Error fetching consultant_events:", e); return { data: [], error: e }; } })(), // RE-ADDED: Busca de eventos do consultor
        ]);
        ...
        if (consultantEventsData.error) console.error("Consultant Events error:", consultantEventsData.error); // RE-ADDED: Log de erro
        ...
        if (configResult.data) {
          ...
          const { checklistStructure, consultantGoalsStructure, interviewStructure, templates, hiringOrigins, salesOrigins, interviewers, pvs } = DEFAULT_APP_CONFIG_DATA;
          ...
          setHiringOrigins(hiringOrigins);
          setSalesOrigins(salesOrigins);
        ...
        setConsultantEvents(consultantEventsData?.data || []); // RE-ADDED: Define eventos do consultor
  ...
  useEffect(() => {
    ...
    const tasksChannel = supabase
        ...
        .on('postgres_changes', { event: '*', schema: 'public', table: 'lead_tasks' }, (payload) => {
            ...
            const newTaskData: LeadTask = {
                ...
                meeting_start_time: payload.new.meeting_start_time, // RE-ADDED: Meeting fields
                meeting_end_time: payload.new.meeting_end_time,   // RE-ADDED: Meeting fields
                manager_id: payload.new.manager_id, // RE-ADDED: Meeting fields
                manager_invitation_status: payload.new.manager_invitation_status, // RE-ADDED: Meeting fields
                updated_at: payload.new.updated_at, // RE-ADDED: updated_at
            };
    ...
    return () => {
        ...
        supabase.removeChannel(consultantEventsChannel); // RE-ADDED: Remove consultant events channel
    };
  }, [user, crmOwnerUserId]);
  ...
  const updateCrmLeadStage = useCallback(async (leadId: string, newStageId: string) => {
    ...
    const updatedLead = { ...lead, stage_id: newStageId, updated_by: user.id };
  ...
  const toggleDailyChecklistCompletion = useCallback(async (daily_checklist_item_id: string, date: string, done: boolean, consultant_id: string) => {
    ...
    const { data, error } = await supabase.from('daily_checklist_completions')
      .upsert(
        ...
        {
          onConflict: 'daily_checklist_item_id,consultant_id,date',
  ...
  const updateLeadMeetingInvitationStatus = useCallback(async (taskId: string, status: 'accepted' | 'declined') => {
    ...
    const { data, error } = await supabase
      .from('lead_tasks')
      .update({ manager_invitation_status: status })
      .eq('id', taskId)
      .eq('manager_id', user.id)
      .select('*')
      .maybeSingle();
    if (error) throw error;
    setLeadTasks(prev => prev.map(task => task.id === taskId ? (data || task) : task));
  ...
  const value: AppContextType = {
    ...
    formCadastros,
    formFiles,
    consultantEvents, // RE-ADDED: ConsultantEvent
    notifications,
    theme,
