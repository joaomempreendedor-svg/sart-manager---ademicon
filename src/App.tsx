...
const RequireAuth: React.FC<{ allowedRoles: UserRole[] }> = ({ allowedRoles }) => {
  ...
  // Evita bloquear a rota raiz ("/") pelo isDataLoading; apenas isAuthLoading bloqueia.
  // Isso elimina o "loading infinito" ao redirecionar imediatamente a partir de Home.
  const isRoot = location.pathname === '/';
  if (isAuthLoading || (!isRoot && isDataLoading)) {
    return <AppLoader />;
  }
