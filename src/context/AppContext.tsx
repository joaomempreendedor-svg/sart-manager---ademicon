...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const addVideoToOnboardingSession = useCallback(async (sessionId: string, title: string, file: File) => { if (!user) throw new Error("Usuário não autenticado."); const session = onboardingSessions.find(s => s.id === sessionId); if (!session) throw new Error("Sessão não encontrada."); const filePath = `onboarding/${sessionId}/${crypto.randomUUID()}-${file.name}`; const { error: uploadError } = await supabase.storage.from('onboarding_videos').upload(filePath, file); if (uploadError) throw uploadError; const { data: urlData } = supabase.storage.from('onboarding_videos').getPublicUrl(filePath); const newVideoData = { session_id: sessionId, title, video_url: urlData.publicUrl, order: session.videos.length + 1 }; const { data: dbData, error: dbError } = await supabase.from('onboarding_videos').insert(newVideoData).select().single(); if (dbError) { await supabase.storage.from('onboarding_videos').remove([filePath]); throw dbError; } setOnboardingSessions(prev => prev.map(s => s.id === sessionId ? { ...s, videos: [...s.videos, dbData] } : s)); }, [user, onboardingSessions]);
  const deleteVideoFromOnboardingSession = useCallback(async (videoId: string, videoUrl: string) => { if (!user) throw new Error("Usuário não autenticado."); const filePath = videoUrl.split('/onboarding_videos/')[1]; await supabase.storage.from('onboarding_videos').remove([filePath]); const { error } = await supabase.from('onboarding_videos').delete().match({ id: videoId }); if (error) throw error; setOnboardingSessions(prev => prev.map(s => ({ ...s, videos: s.videos.filter(v => v.id !== videoId) }))); }, [user]);
