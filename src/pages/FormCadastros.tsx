...
import { FormCadastroDetailModal } from '@/components/FormSubmissionDetailModal';
import toast from 'react-hot-toast';
import { TableSkeleton } from '@/components/TableSkeleton';
import { formSteps } from '@/data/formStepsData'; // Importar formSteps

export const FormCadastros = () => {
  const { formCadastros, formFiles, isDataLoading, deleteFormCadastro } = useApp();
  const [copiedLink, setCopiedLink] = useState(false);
  const [searchTerm, setSearchTerm] = useState('');
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [selectedCadastro, setSelectedCadastro] = useState<FormCadastro | null>(null);
  ...
  const filteredCadastros = useMemo(() => {
    if (!searchTerm) return formCadastros;
    const lowerCaseSearchTerm = searchTerm.toLowerCase();
    return formCadastros.filter(cadastro => 
      Object.values(cadastro.data).some(value => 
        String(value).toLowerCase().includes(lowerCaseSearchTerm)
      ) || (cadastro.internal_notes && cadastro.internal_notes.toLowerCase().includes(lowerCaseSearchTerm))
    );
  }, [formCadastros, searchTerm]);

  const handleOpenDetails = (cadastro: FormCadastro) => {
    setSelectedCadastro(cadastro);
    setIsModalOpen(true);
  };

  const handleDeleteCadastro = async (cadastroId: string, clientName: string) => {
    if (window.confirm(`Tem certeza que deseja excluir o cadastro de "${clientName}" e todos os seus arquivos? Esta ação não pode ser desfeita.`)) {
      try {
        await deleteFormCadastro(cadastroId);
    ...
    <div className="p-4 sm:p-8 max-w-7xl mx-auto min-h-screen">
      ...
      <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm mb-8">
        ...
        <div className="flex flex-col sm:flex-row items-center space-y-3 sm:space-y-0 sm:space-x-3">
          <input
            ...
            className="flex-1 p-2 bg-gray-50 dark:bg-slate-700 border border-gray-200 dark:border-slate-600 rounded-lg text-gray-800 dark:text-gray-200 text-sm font-mono w-full"
          />
          <button
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200 dark:border-slate-700 flex flex-col sm:flex-row justify-between items-start sm:items-center">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Cadastros Recebidos ({formCadastros.length})</h2>
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-slate-700">
              {filteredCadastros.length === 0 ? (
                ...
                filteredCadastros.map(cadastro => {
                  const cadastroFiles = formFiles.filter(f => f.submission_id === cadastro.id);
                  const hasFiles = cadastroFiles.length > 0;
                  const clientName = cadastro.data.nome_completo || 'N/A';

                  return (
                    <tr key={cadastro.id} className="hover:bg-gray-50 dark:hover:bg-slate-700/50 transition">
                      <td className="px-6 py-4 font-medium text-gray-900 dark:text-white">{clientName}</td>
                      <td className="px-6 py-4">{cadastro.data.email || 'N/A'}</td>
                      <td className="px-6 py-4">{new Date(cadastro.submission_date).toLocaleDateString('pt-BR')}</td>
                      <td className="px-6 py-4">
                        {hasFiles ? (
                          <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300">
                            <FileText className="w-3 h-3 mr-1" /> {cadastroFiles.length} Arquivo(s)
                          </span>
                        ) : (
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex justify-end items-center space-x-2">
                          <button onClick={() => handleOpenDetails(cadastro)} className="p-2 text-gray-400 hover:text-blue-500 rounded-full" title="Ver Detalhes">
                            <Eye className="w-4 h-4" />
                          </button>
                          <button onClick={() => handleDeleteCadastro(cadastro.id, clientName)} className="p-2 text-gray-400 hover:text-red-500 rounded-full" title="Excluir Cadastro">
