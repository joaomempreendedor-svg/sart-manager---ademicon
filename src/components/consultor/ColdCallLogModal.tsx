...
interface ColdCallLogModalProps {
  ...
  onCreateCrmLeadFromColdCall: (coldCallLead: ColdCallLead, meeting?: { date?: string; time?: string; modality?: string; notes?: string }, coldCallResult?: ColdCallResult) => void;
}
...
export const ColdCallLogModal: React.FC<ColdCallLogModalProps> = ({
  ...
  const handleSaveLogAndStage = async (): Promise<boolean> => {
    ...
    try {
      ...
      else if (callResult === 'Pedir retorno' || callResult === 'Não atendeu' || callResult === 'Não chamou' || callResult === 'Número inválido' || callResult === 'Sem interesse') newStageValue = 'Tentativa de Contato';
