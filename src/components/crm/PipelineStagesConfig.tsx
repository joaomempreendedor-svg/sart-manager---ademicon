...
import { Plus, Edit2, ArrowUp, ArrowDown, ToggleLeft, ToggleRight, CheckCircle, XCircle, Trophy, Skull } from 'lucide-react';
import StageModal from './StageModal';

const PipelineStagesConfig = () => {
  const { crmPipelines, crmStages, updateCrmStage, updateCrmStageOrder } = useApp();
            ...
            <li key={stage.id} className={`p-4 flex items-center justify-between group ${!stage.is_active ? 'bg-gray-50 dark:bg-slate-800/50 opacity-60' : ''}`}>
              <div className="flex items-center space-x-4">
                <div className="flex flex-col space-y-1">
                  <button onClick={() => handleMove(index, 'up')} disabled={index === 0} className="p-1 text-gray-400 hover:text-brand-600 disabled:opacity-30"><ArrowUp className="w-4 h-4" /></button>
                  <button onClick={() => handleMove(index, 'down')} disabled={index === pipelineStages.length - 1} className="p-1 text-gray-400 hover:text-brand-600 disabled:opacity-30"><ArrowDown className="w-4 h-4" /></button>
                </div>
                <div>
              ...
              <div className="flex items-center space-x-2 opacity-0 group-hover:opacity-100 transition-opacity">
                <button onClick={() => handleToggleActive(stage)} className={`p-2 rounded-full ${stage.is_active ? 'text-gray-400 hover:text-yellow-600' : 'text-gray-400 hover:text-green-600'}`} title={stage.is_active ? 'Desativar' : 'Ativar'}>
                  {stage.is_active ? <ToggleLeft className="w-5 h-5" /> : <ToggleRight className="w-5 h-5" />}
                </button>
                <button onClick={() => handleEdit(stage)} className="p-2 text-gray-400 hover:text-blue-500 rounded-full"><Edit2 className="w-4 h-4" /></button>
              </div>
            </li>
          ))}
        </ul>
